...
if (0) {
	int i;
	for (i = 1; i < 0x70; i++) {
    //	SX1276Read( i, &SX1276Regs[i] );
		printk (KERN_ALERT "%d ", SX1276Regs[i]);
	}
}
...
if (0) {
	int i;
	for (i = 1; i < 0x70 - 1; i++) {
		printk (KERN_ALERT "0x%02x ", SX1276Regs[i]);
	}
}
...
//printk(KERN_ALERT "SX1276 VERSION: %d\n", SX1276LR->RegVersion);
}
...
int SX1276LoRaReadRssi( void )
{
    ...
    if( LoRaSettings.RFFrequency < 860000000 )  // LF
    {
        return RSSI_OFFSET_LF + ( int )SX1276LR->RegRssiValue;
    }
    else
    {
        return RSSI_OFFSET_HF + ( int )SX1276LR->RegRssiValue;
    }
}
int SX1276LoRaGetPacketRssi( void )
{
    return RxPacketRssiValue;
}
...
#else
                RxPacketRssiValue = RSSI_OFFSET_LF + ( ( int )SX1276LR->RegPktRssiValue ) + RxPacketSnrEstimate;
#endif
            }
            else
            {
//                RxPacketRssiValue = RSSI_OFFSET_LF + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
                RxPacketRssiValue = RSSI_OFFSET_LF + SX1276LR->RegPktRssiValue;
            }
        }
        else                                        // HF
        {    
...
#else
                RxPacketRssiValue = RSSI_OFFSET_HF + ( ( int )SX1276LR->RegPktRssiValue ) + RxPacketSnrEstimate;
#endif
            }
            else
            {    
//                RxPacketRssiValue = RSSI_OFFSET_HF + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
                RxPacketRssiValue = RSSI_OFFSET_HF + SX1276LR->RegPktRssiValue;
            }
        }
...
static void deinit_trigger(void *arg)
{
	free_irq( gpio_to_irq(DIO0_GPIO), arg);
}

#if 0
int SX1276ReadBlock(struct rf_data *rf, int len, uint32_t timeout)
{
		...
		init_trigger((void *)rf);
        /* RX single mode */
        SX1276LoRaSetOpMode( RFLR_OPMODE_RECEIVER_SINGLE );

/*-------------------------------------------------------------------------*/
	
//	wait_for_completion_interruptible_timeout(&rf->done, timeout);
		(void) timeout;
		res =  wait_for_completion_interruptible(&rf->done);
       	deinit_trigger((void *)rf);
//		SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
...
/*-------------------------------------------------------------------------*/
        ...
        if( LoRaSettings.ImplicitHeaderOn == true )
        {
            RxPacketSize = min(len, (int)SX1276LR->RegPayloadLength);
            SX1276ReadFifo( rf->buffer, (int)SX1276LR->RegPayloadLength );
        }
        else
        {
            SX1276Read( REG_LR_NBRXBYTES, &SX1276LR->RegNbRxBytes );
            RxPacketSize = min(len, (int)SX1276LR->RegNbRxBytes);
            SX1276ReadFifo( rf->buffer, SX1276LR->RegNbRxBytes );
        }
...
int SX1276WriteBlock(struct rf_data *rf, int len, uint32_t timeout)
{
		int res;
		int TxPacketSize = min(len, (int)LoRaSettings.PayloadLength);
        ...
        // Write payload buffer to LORA modem
        SX1276WriteFifo( rf->buffer, SX1276LR->RegPayloadLength );
...
//		SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
		...
		if (res) {
			printk(KERN_ALERT "wait for completion error!\n");
        	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
			return -ERESTARTSYS;
		}
...
//		SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
        ...
        {    
            ...
            else
            {
                ...
                RxPacketRssiValue = RSSI_OFFSET_LF + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
#else
                RxPacketRssiValue = RSSI_OFFSET_LF + ( 10666 * ( ( int )SX1276LR->RegPktRssiValue ) ) / 10000;
#endif
            }
        }
        else                                        // HF
        {    
            ...
            {    
                ...
                RxPacketRssiValue = RSSI_OFFSET_HF + ( 1.0666 * ( ( double )SX1276LR->RegPktRssiValue ) );
#else
                RxPacketRssiValue = RSSI_OFFSET_HF + ( 10666 * ( ( int )SX1276LR->RegPktRssiValue ) ) / 10000;
#endif
            }
        }
        ...
        if( LoRaSettings.RxSingleOn == true ) // Rx single mode
        {
            ...
            {
            	RxPacketSize = min(len, (int)SX1276LR->RegPayloadLength);
                SX1276ReadFifo( rf->buffer, SX1276LR->RegPayloadLength );
            }
            else
            {
                SX1276Read( REG_LR_NBRXBYTES, &SX1276LR->RegNbRxBytes );
...
#else
	/* lichen: CAD use DIO0, DIO1 */
                                    // CadDone                   CAD Detected
        SX1276LR->RegDioMapping1 = RFLR_DIOMAPPING1_DIO0_10 | RFLR_DIOMAPPING1_DIO1_10 | RFLR_DIOMAPPING1_DIO2_11 | RFLR_DIOMAPPING1_DIO3_11;
        SX1276LR->RegDioMapping2 = RFLR_DIOMAPPING2_DIO4_11 | RFLR_DIOMAPPING2_DIO5_11;
#endif
...
//	wait_for_completion_interruptible_timeout(&rf->done, timeout);
		...
		(void) timeout;
		if ( wait_for_completion_interruptible(&rf->done) ) {
        	SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );
			return -ERESTARTSYS;
		}
