...
#else
	result = ((uint64_t) freq) * (1 << 19);
	do_div(result, FREQ_XOSC);
    freq = ( uint32_t ) result;
//	printk(KERN_ALERT "freq = %u\n", freq);
#endif
...
{
uint8_t buf[3];
printk(KERN_ALERT "Before: Msb=%d, Mid=%d, Lsb=%d\n", SX1276LR->RegFrfMsb, SX1276LR->RegFrfMid, SX1276LR->RegFrfLsb);
SX1276ReadBuffer( REG_LR_FRFMSB, buf, 3 );
printk(KERN_ALERT "After : Msb=%d, Mid=%d, Lsb=%d\n", buf[0], buf[1], buf[2]);
}
#endif
...
#else
	result = ((uint64_t)LoRaSettings.RFFrequency) * FREQ_XOSC;
	do_div(result, (1 << 19));
    LoRaSettings.RFFrequency = ( uint32_t ) result;
#endif
